...
cleaned_uboats = cleaned_uboats %>%
  mutate(Active_Service = ifelse(Commissioned < as.Date("1914-07-28"), # if commissioned before the start of WW1
                                 ifelse(Fate_Date < as.Date("1918-11-11"), # if sunk before the end of WW1
                                        difftime(Fate_Date, as.Date("1914-07-28"), units = "days"), # calculate lifespan from start of WW1
                                        ifelse(
