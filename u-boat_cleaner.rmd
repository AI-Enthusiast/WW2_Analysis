...
```{r}
# convert fate date and commission date to date (eg 1935-07-25)
cleaned_uboats = cleaned_uboats %>%
        mutate(Commissioned = as.Date(Commissioned, format = "%d %B %Y"),
               Fate_Date = as.Date(Fate_Date, format = "%d %B %Y"))
```
```{r}
# cleaned_uboats = cleaned_uboats %>%
#   mutate(Active_Service = calc_war_service(as.Date(Commissioned, format = "%d %B %Y"), as.Date(Fate_Date, format = "%d %B %Y")))
for (i in 1:nrow(cleaned_uboats)) {
  cleaned_uboats$Active_Service[i] = calc_war_service(cleaned_uboats$Commissioned[i], cleaned_uboats$Fate_Date[i])
}
write_csv(cleaned_uboats, "uboats_cleaned.csv")
```
```{r}
